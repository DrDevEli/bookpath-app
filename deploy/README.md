...
running nginx + Node (PM2) + MongoDB + Redis. Canonical domain: **bookpath.online**.
...
```
                      ┌─────────────────────────────────────────┐
                      │              nginx  :80/:443            │
   bookpath.online ──▶│  /, /static/        → SPA (build/)       │
...
1. **Renew `bookpath.online`** in Namecheap (auto-renew ON). It expires 21 Aug.
...
1. **DNS (Namecheap):** add A records `bookpath.online` → VPS IP and `www` → VPS IP.
...
3. **SSL:** `certbot --nginx -d bookpath.online -d www.bookpath.online`
   (run after DNS propagates).
4. **Indexing:** submit `https://bookpath.online/sitemap.xml` in Google Search Console.
...
canonical domain is `bookpath.online`. All URLs (sitemap, canonical, CORS,
User-Agent) are switched to `bookpath.online`; the backend still honours a
